...
<CONTEXT>
We inject product data and the user's basic profile details below. You can answer questions related to:
- **Available Products**: What is in stock, pricing, categories.
- **User Profile**: The current user's requested items, or payout rate.
</CONTEXT>

RULES:
1. Always be polite, concise, and act like a customer support agent.
2. If the user uploads an image of a product, help them identify if we have something similar in our register.
3. DO NOT output JSON. This AI is purely conversational to help the user. Answer in readable markdown text.
4. If they ask about things you don't have access to (like store revenue or other users), kindly mention you don't have access to admin data.
5. List products using ordered numbered lists, NEVER bullet points.
6. EXTREMELY IMPORTANT: When listing or recommending products, ALWAYS state the product name clearly. Format each product link EXACTLY like this HTML anchor:
   <a href="#" class="ai-product-link" data-product-name="Exact Product Name">Exact Product Name</a>
   NEVER skip or omit the product name. The name field in the data is called "name".
7. IF the user EXPLICITLY asks to order, buy, or request a specific quantity of an item that you have in the store, output a JSON block AT THE END of your text response.
   JSON SCHEMA:
   {
   ...
   Use this ONLY when they are ordering an item by quantity. Provide brief text before the JSON.
`;


let creditAiChatThreads = [];
let currentCreditChatId = null;

window._sendCreditAiSuggested = function(text) {
...
window.openCreditAiModal = function() {
    ...
    fetch('menu-buttons/credit-ai/credit-ai.html?v=' + Date.now())
        .then(res => res.text())
        .then(html => {
...
function initCreditAiChatLogic() {
    ...
    async function handleSend(isEdit = false, spliceIndex = null) {
        ...
        const activeThread = creditAiChatThreads.find(t => t.id === currentCreditChatId);
...
-----------------------------\n`;
    ...
    window._handleAiProductSelection = function(productName, initialQty = 1, existingRequestID = null, msgIndex = null) {
        ...
        if (prod) {
            ...
            buyBtn.addEventListener('click', function() {
                ...
                if (msgIndex !== null) {
                    const thread = creditAiChatThreads.find(t => t.id === currentCreditChatId);
    ...
    function saveActiveHistory() {
        localStorage.setItem('nd_user_ai_chat_threads', JSON.stringify(creditAiChatThreads));
    ...
    function renderActiveThread(preserveScroll = false, scrollToSearch = false, anchorMsgId = null) {
        ...
        const active = creditAiChatThreads.find(x => x.id === currentCreditChatId);
        if (!active || active.messages.length === 0) {
            let defaultContent = `
                <div class="ai-suggested-container">
                    <div class="ai-suggested-title">Suggested Questions:</div>
                    <button class="ai-suggested-btn" onclick="window._sendCreditAiSuggested('what are the available products?')">1. what are the available products?</button>
                    <button class="ai-suggested-btn" onclick="window._sendCreditAiSuggested('do i have any pending request?')">2. do i have any pending request?</button>
                    <button class="ai-suggested-btn" onclick="window._sendCreditAiSuggested('What should I eat next?')">3. What should I eat next?</button>
                    <button class="ai-suggested-btn" onclick="window._sendCreditAiSuggested('What are all the available features in this app?')">4. What are all the available features in this app?</button>
                    <button class="ai-suggested-btn" onclick="window._sendCreditAiSuggested('what are the latest products?')">5. what are the latest products?</button>
                </div>
            `;
    ...
    function _renderAiPinnedMessage() {
        ...
        const activeThread = creditAiChatThreads.find(x => x.id === currentCreditChatId);
    ...
    function renderSidebar(searchQuery = '') {
        ...
        let sorted = [...creditAiChatThreads].sort((a, b) => b.updatedAt - a.updatedAt);
        ...
        searchResults.forEach(({ thread, matchType, matchPreview, matchMsgIndex }) => {
            const div = document.createElement('div');
            div.className = `thread-item ${thread.id === currentCreditChatId ? 'active' : ''} ${thread.isPinned ? 'pinned' : ''}`;
    ...
    window.confirmAiRename = function () {
        ...
        if (newTitle) {
            const thread = creditAiChatThreads.find(t => t.id === threadToEditId);
            if (thread) {
